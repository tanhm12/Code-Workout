...
def maximumSum(a, m):
    ...
    pref_sum = [0 for _ in range(len(a) + 1)]
    for i in range(len(a)):
        pref_sum[i+1] += (pref_sum[i] + a[i]) % m
    
    tree = SegmentTree(pref_sum, operations=[sum_operation, max_operation])
    length = len(pref_sum)
    for i in range(1, length):
        tree.update_range(i, length-1)
